    ...
    for r,p,d in out_files:
        ...
        if (os.path.isfile(os.path.join(read_file))):
            with open(read_file) as file:
                ...
                times_per_div[case].append(float(read[index:].split()[1]))
    
    return times_per_div

def gen_scaling_plots(div : int, times_per_div : dict):
    fig, ax = plt.subplots()
    ax.grid(True)
    ax.set_title(f"Strong Scaling plot | Divisions of L :{div} ~ h :{10/div:.3f}")
    ...
    for case in times_per_div.keys():
        if int(case.split("-")[1]) == div:
            ...
            set_axis=0
            ax.set_xticks([i for i in range(2,2**len(times_per_div[case])+2,2)])
            procs = [2**i for i in range(1,len(times_per_div[case])+1)]
            plt.plot(procs,times_per_div[case], "-o", label=f"{case}")
...
def gen_step_size_time_plot(out_files : list, times_per_proc_dict : dict):
    ...
    plt.clf()
    fig, ax = plt.subplots()
    ax.set_xticks(range(0,200,16))
    sec = ax.secondary_xaxis(location=1)
    sec.set_xticks(range(16,200,16))
    sec.set_xticklabels(labels=[f"\n{i:.3f}" for i in 10/np.arange(16,200,16)], 
                        rotation=10,
                        fontsize=8)
